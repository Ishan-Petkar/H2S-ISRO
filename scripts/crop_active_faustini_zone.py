"""Crops the official DFSAR mask around a known active ice region near Faustini crater."""
import rasterio
import numpy as np
import os
...
def main():
    ...
    with rasterio.open(mask_path) as src:
        # Known active point at row=6047, col=8417
        row_center, col_center = 6047, 8417
        ...
        print(f"=== Active Faustini Region Mask Crop ===")
        print(f"Crop shape: {crop.shape}")
        print(f"Ice pixels: {ice_pixels} / {total_pixels} ({pct:.4f}%)")
        
        if ice_pixels > 0:
            print("Found ice pixels inside active Faustini crop!")
            out_dir = os.path.join(os.path.dirname(__file__), "..", "results")
            os.makedirs(out_dir, exist_ok=True)
            out_path = os.path.join(out_dir, "faustini_active_ice_mask_crop.npy")
