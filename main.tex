...
To ground the Gaussian reference model in physical measurements, we extracted dry-regolith baseline statistics from public LRO Mini-RF S-band CPR polar mosaics. Over south-polar highland craters (excluding fresh craters with blocky ejecta), the sunlit, ice-free regolith background displays a circular polarization ratio of $\mu_R^{\CPR} = 0.35$ with standard deviation $\sigma_R^{\CPR} = 0.08$. The degree of polarization is characterized by $\mu_R^{\DOP} = 0.75$ with standard deviation $\sigma_R^{\DOP} = 0.08$. Within permanently shadowed regions (PSRs) that are free of anomalous volatile enhancements (e.g., portions of the Cabeus and Shackleton floor backgrounds), the values are similarly characterized by $\mu_R^{\CPR} = 0.32 \pm 0.10$ and $\mu_R^{\DOP} = 0.78 \pm 0.09$, establishing a consistent and stable polar reference baseline. Wavelength-scale roughness coupling yields a physical correlation coefficient of $r_{\mathcal{R}} \approx -0.15$ under dry conditions.

\textit{Choice of $2\sigma$ threshold.}
Under the Gaussian reference model, a $2\sigma$ threshold yields marginal tail probabilities of $P(z_{\CPR} \ge 2) \approx 0.0228$ and $P(z_{\DOP} \le -2) \approx 0.0228$. Although raw radar backscatter coefficients follow non-Gaussian statistics (such as Rayleigh or Gamma distributions), the log-transformed multilooked CPR and DOP values converge to a Gaussian distribution by the Central Limit Theorem after spatial filtering over the $7 \times 7$ Refined-Lee speckle filter window. Thus, under the independent reference model ($r_{\mathcal{R}} = 0$), the joint probability of a dry-regolith pixel falsely satisfying the AND condition at Stage~2 is approximately $0.0228^2 \approx 5.18 \times 10^{-4}$. Accounting for the empirical negative correlation ($r_{\mathcal{R}} = -0.15$), the joint tail probability in opposite quadrants is $1.10 \times 10^{-3}$ (formally derived in Lemma~\ref{lem:fp} and Appendix~\ref{app:proof:lemma1}). For a typical PSR scene size of $N \sim 7 \times 10^4$ pixels (matching Shackleton's floor), this corresponds to roughly 77 raw false-positive candidates, which is well within the capacity of Stages 3--5 to filter down to zero. The $2\sigma$ level is selected to balance this raw candidate generation against recall sensitivity. Ablation over thresholds from $1.5\sigma$ to $3.0\sigma$ is reported in Section~\ref{sec:experiments}.
...
To validate the joint planner physically, we extracted actual historical Chandrayaan-2 polar orbit Two-Line Elements (TLEs) and propagated them via the SGP4 physical model over a candidate Shackleton crater coordinate ($89.9^\circ$S). For a continuous tracking window, the maximum orbital blackout period (when elevation dropped below $5^\circ$) bounded the relay constraint at $T_{\mathrm{relay}} = 1.63$ hours. We generated a high-fidelity synthetic polar DEM (5\,m/px) mimicking a steep, cratered PSR. For a VIPER-class rover ($E_{\max} = 5.0$\,kWh) executing a three-stop sampling excursion, the priority-queue A* planner computed an optimal path traversing 355 meters. The excursion strictly satisfied the $1.63$-hour relay bound and consumed 995\,Wh ($19.9\%$ of $E_{\max}$), maintaining a comfortable $>3.0$\,kWh contingency margin above the mandatory $20\%$ baseline.
...
\label{eq:fpbound}
\end{equation}
where $f(x,y; r_{\mathcal{R}})$ is the standard bivariate normal PDF. Under the independent baseline ($r_{\mathcal{R}} = 0$), $P_{\mathrm{FP},\mathrm{indep}}^{(2)} \approx 5.18 \times 10^{-4}$. For a physical negative correlation $r_{\mathcal{R}} = -0.15$ (arising from roughness-induced depolarization), the rate is $1.10 \times 10^{-3}$.
\end{lemma}

\begin{proof}
Under the bivariate Gaussian assumption, the marginal tail probabilities are $P(z_{\CPR} \ge 2) = 1 - \Phi_{\mathcal{N}}(2) \approx 0.0228$ and $P(z_{\DOP} \le -2) = \Phi_{\mathcal{N}}(-2) \approx 0.0228$. Under independence ($r_{\mathcal{R}} = 0$), the joint probability is the product of these marginals: $P_{\mathrm{FP},\mathrm{indep}}^{(2)} \approx 0.0228^2 \approx 5.18 \times 10^{-4}$. When a physical negative correlation $r_{\mathcal{R}} = -0.15$ is introduced, the joint tail probability in opposite quadrants increases to $1.10 \times 10^{-3}$, representing the physical limit of raw anomaly generation before spatial and roughness vetoes are applied.
\end{proof}
...
\end{equation*}
where $f(x, y; r_{\mathcal{R}})$ is the standard bivariate normal PDF and $\phi_{\mathcal{N}}$ is the standard univariate normal PDF. For an empirical correlation coefficient $r_{\mathcal{R}} = -0.15$ (derived from public LRO Mini-RF S-band polar observations), the actual false-positive rate is $1.10 \times 10^{-3}$, which is slightly higher than the independent case of $5.18 \times 10^{-4}$. This negative correlation increases the rate of raw anomalous pixels, making the subsequent spatial coherence and roughness vetoes (Stages 3--5) of the cascade critical to suppress these correlated anomalies and guarantee a final false-positive rate well below $0.1\%$.
\end{proof}
