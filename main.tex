...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{scripts/results/bivariate_anomaly_space.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{scripts/results/dbscan_scatter.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{scripts/results/traverse_3d.pdf}
\caption{Optimal rover traverse path (cyan) plotted over the LOLA DEM. The 3D surface is coloured by the combined A* risk and slope cost map $C(p)$, showing how the planner navigates around high-slope crater rims and high-risk regolith while routing from the safe sunlit lander (white) to the shadowed ice targets (magenta).}
\label{fig:traverse_3d}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{scripts/results/energy_profile.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{scripts/results/datacube.pdf}
\caption{Exploded view of the multi-modal datacube on the real DFSAR test crop. Layer 1 (bottom) is the LOLA DEM. Layer 2 maps the raw L-band CPR. Layer 3 maps the derived physical slope and roughness constraints. Layer 4 (top) shows the final subset of pixels ($\Pi > 0$) that survived the 5-stage cascade detector. The vertical alignment demonstrates how spatial coherence and surface roughness parameters strictly limit the final candidate footprint.}
\label{fig:datacube}
\end{figure}
