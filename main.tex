...
residual uncertainty is explicitly quantified rather than suppressed,
...
Mini-RF lunar ice interpretations~\cite{spudis2013mini,fassett2015}.
...
\begin{itemize}
  ...
  \item \textbf{Physically-derived penetration depth}: skin-depth theory
    yields a 4.2\,m L-band estimate; a 2\,m conservative column is
...
Section~\ref{sec:experiments} presents the experimental evaluation plan.
...
Fassett and Thomson~\cite{fassett2015} argued that roughness-driven
...
$\Pi(p)$ & Ice probability score at pixel $p$ \\
...
\STATE Assign $\Pi(p) \in [0,1]$ via logistic combination of
       $z_{\CPR}$, $z_{\DOP}$, and cluster-level $r$.
\STATE Assign $\Phi(p)$ encoding roughness-veto uncertainty +
       baseline-bias risk.
...
Under the Gaussian reference model, $P(z \ge 2) \approx 0.023$,
so the joint probability of a dry-regolith pixel falsely satisfying
...
further.
The $2\sigma$ level is chosen as the lowest threshold that keeps this
...
small DSC floor patch. Each row shows surviving pixels after the named
stage. The roughness veto (Stage~3) removes ejecta; DBSCAN (Stage~4)
removes isolated single-pixel anomalies; dual-frequency corroboration
(Stage~5) retains only candidates consistent with a volume scattering
mechanism. Percentages are representative values; actual counts are
scene-dependent.}
\label{fig:stages}
\end{figure}
...
This mask, while physically grounded, is limited in spatial
generalisation: it cannot reliably extrapolate ice-probability
estimates to crater morphologies not represented in the reference
population $\mathcal{R}$.
...
The U-Net does not replace the AND-cascade; it refines it.
...
\label{eq:fusion}
\end{equation}
ensuring that a pixel must be flagged as anomalous by both
the physics-based cascade and the learned model to receive
a high final probability score.
...
Cyan patches represent ice-bearing regolith detected by the AND-cascade.
...
$s(p) \le 5^\circ$, rock cover $< 5\%$ (OHRC-derived; constraints
consistent with Chandrayaan-3 landing site selection criteria~\cite{kumar2022dfsar}),
...
Visibility windows are computed from the orbiter's published
Two-Line Element (TLE) set~\cite{celestrak2024} using the
\texttt{skyfield} Python library~\cite{skyfield2019},
which propagates the SGP4 orbital model forward over the
...
Where TLE data are unavailable, a conservative 2-hour visibility
cadence for a representative Chandrayaan-2-class polar orbit
is used as a stated assumption and labeled as such in $\Phi$.
...
\begin{tikzpicture}[x=1.0cm, y=0.75cm, >=Stealth, font=\small]
  ...
  % Phase labels
  \node at (0.6, 0.50)  {Pre-charge};
  \node at (1.7, 0.50)  {Ingress};
  \node at (4.1, 0.50)  {In-shadow excursion};
  \node at (6.5, 0.50)  {Egress};
  \node at (7.9, 0.50)  {Recharge};
...
\label{eq:icefrac}
\end{equation}
with coefficients $a, b$ from~\cite{sahu2025integrated}.
...
\label{eq:fpbound}
\end{equation}
assuming independence between CPR and DOP anomalies conditional on
no ice.
\end{lemma}
...
$P_{\mathrm{FP}}^{(2)}$.
The AND-cascade property guarantees that adding stages never increases
...
\begin{equation}
\Fice^{\min} = e^{\,2\sigma_R^{\CPR}/a},
\label{eq:ficemin}
\end{equation}
where $a$ is the empirical CPR-to-ice-fraction coefficient
from Eq.~(\ref{eq:icefrac}).
\end{theorem}
...
monotonically increasing in CPR for $a > 0$; inverting at
$\CPR = \mu_R^{\CPR} + 2\sigma_R^{\CPR}$ yields
Eq.~(\ref{eq:ficemin}).
...
Table~\ref{tab:metrics} lists primary evaluation metrics.
...
Upon data availability, Table~\ref{tab:results} will be populated with
empirical precision, recall, F$_1$, and false-positive rate values
following the evaluation protocol in Section~\ref{sec:experiments}.
...
\begin{table}[htbp]
\caption{Results Table (To Be Populated After Data Acquisition)}
\label{tab:results}
\centering
\small
...
\toprule
Method & Prec. & Recall & F$_1$ & FPR \\
\midrule
Global threshold~\cite{kumar2022dfsar} & -- & -- & -- & -- \\
Single-freq anomaly & -- & -- & -- & -- \\
Proposed (cascade only) & -- & -- & -- & -- \\
Proposed (cascade+UNet) & -- & -- & -- & -- \\
\midrule
\multicolumn{5}{l}{\footnotesize Populated upon experimental execution.} \\
\bottomrule
\end{tabular}
\end{table}
...
\addlegendentry{Precision (illustrative)}
\addplot[red, thick, dashed] coordinates {
  (1.5,0.91)(2.0,0.83)(2.5,0.76)(3.0,0.64)
};
\addlegendentry{Recall (illustrative)}
\addplot[black, thick, dashed] coordinates {
  (1.5,0.71)(2.0,0.78)(2.5,0.79)(3.0,0.74)
};
\addlegendentry{F$_1$ (illustrative)}
\end{axis}
\end{tikzpicture}
\caption{Precision-recall tradeoff as roughness veto threshold varies.
Dashed curves are illustrative; solid curves will replace them upon
experimental execution. The proposed default of $2\sigma$ balances
...
five AND-cascade stages (normalised to PSR pixel count). Values are
illustrative; actual counts depend on scene-specific roughness and
ejecta distribution.}
...
This section is a mandatory first-class deliverable, not an appendix.
...
\begin{enumerate}
  ...
  \item \textbf{Inter-crater calibration transfer}: Empirical
    ...
    applying them to a new DSC introduces unquantifiable transfer error
    beyond the stated $\pm$15--20\%.
\end{enumerate}
...
\subsection{Proof of Theorem~\ref{thm:feasibility}}
...
\begin{itemize}
  \item[\checkmark] All datasets are publicly available (PRADAN, PDS,
    LRO archive).
  \item[\checkmark] Preprocessing steps are fully specified with tool
    versions (MIDAS v2.1, SNAP 9.0).
  ...
  \item[\checkmark] Code skeleton available at
    \texttt{github.com/IshanPetkar/H2S-ISRO}.
