...
\begin{equation}
\Pi_{\mathrm{cascade}}(p) = \bigl(1 + \exp(-\eta(p))\bigr)^{-1},
\label{eq:logistic}
\end{equation}
where $\eta(p) = \theta_0 + \theta_1 z_{\CPR}(p) + \theta_2 z_{\DOP}(p) + \theta_3 r(p)$, and $\theta_0 = -2.0$, $\theta_1 = 1.5$, $\theta_2 = -1.5$, and $\theta_3 = 1.0$ are default calibration coefficients chosen to map $z$-scores to $[0, 1]$ probability space.
