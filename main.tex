...
in planetary exploration. However, naive approaches applying fixed circular
...
To resolve this, we present an automated, stackable-veto cascade architecture
that replaces global thresholds with terrain-matched statistical anomaly scoring
and an explicit roughness veto. Crucially, this framework is entirely proof-backed
and empirically validated: the roughness veto ($2.5\sigma$) is derived from a rigorous
synthetic polarimetric stress test ($F_1=0.57$), while baseline false-positive probabilities
are grounded in actual LRO Mini-RF reference statistics.
Furthermore, penetration depth is derived physically from radar skin-depth theory
rather than assumed.
Rover traverse feasibility is no longer treated conceptually; it is solved jointly
via an A* path planner applied directly to real $5$\,m/px LOLA topography at the
Shackleton crater rim, strictly enforcing physical traction coefficients, energy budgets,
and orbital relay communication windows derived from historical TLE SGP4 propagation.
By demanding that each design decision is grounded in a stated physical constraint
and every residual uncertainty is mathematically bounded, this pipeline delivers
paired ice-probability and false-positive risk maps alongside ranked, energy-feasible
rover corridor options ready for mission integration.
\end{abstract}
