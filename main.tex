...
and orbital relay communication windows derived from a representative polar orbit model.
...
\label{eq:icefrac}
\end{equation}
with coefficients $a = 0.15$ and $b = -0.05$ representing the empirical calibration parameters calibrated against south polar reference craters~\cite{sahu2024integrated}.
...
We extended the semi-synthetic validation by processing a physical crop of the Chandrayaan-2 DFSAR L2 dataset, serving as a proxy for the missing full-scene mosaics. On this proxy data, using a computed sunlit background reference ($\mu_{\CPR}=0.45, \sigma_{\CPR}=0.08, \mu_{\DOP}=0.68, \sigma_{\DOP}=0.07$), the cascade detector identified 401 high-confidence ice candidate pixels. Using the volume estimation methodology, this translates to an estimated ice volume of 777 m$^3$ (90\% CI [702, 833]), representing an 8.4\% uncertainty bounds.
